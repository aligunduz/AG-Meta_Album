""" Scalar-Gated MAML: an independent copy of the FO-MAML baseline.

Each fast-weight tensor has one static sigmoid gate shared across tasks and
inner steps. Query loss learns both the initialization and the gate logits;
...
class MyLearner(Learner):
    ...
    def __init__(self, 
                 ...
                 weights: List[torch.Tensor] = [],
                 maml_params: dict = {},
                 gate_logits: List[torch.Tensor] = None) -> None:
        ...
        if self.ncc:
            ...
            for _ in range(self.T):     
                ...
                task_weights = update_weights(task_weights, grads, 
                    self.grad_clip, self.lr, self.gate_logits)
