...
def update_weights(weights: List[torch.Tensor],
    ...
    """ Apply one task-conditioned scalar gate per fast-weight tensor.

    The contract is identical to SGMAML: one scalar logit per fast-weight
    tensor, turned into a multiplier by a sigmoid. The only difference is how
    the logits are produced upstream. SGMAML passes the shared meta-parameter
    a_j directly, while TCSGMAML passes a_j + delta_ij, where delta_ij is the
    GateNet correction computed once per episode from the support-only task
    embedding. Both are scalars and neither is a fast weight.
    ...
    Args:
        ...
        gate_logits (List[torch.Tensor]): Per-task scalar gate logits. These
            are never part of the support-gradient inputs or updated here, and
            the same values are reused by every inner step of one episode.
