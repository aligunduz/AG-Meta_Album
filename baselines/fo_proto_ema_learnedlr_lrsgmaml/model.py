"""Independent FO-Proto-EMA-LearnedLR-LRSGMAML submission using the repository FOMAML protocol."""
import json
import random
...
def validate_config(config):
    ...
    if config["constant_condition"]["init"] != "ema":
        raise ValueError("Learned LR experiment requires EMA")
    if config["method_config"]["inner_steps"] != 5 or config["method_config"]["classifier_lr"] != 0.01:
        raise ValueError("Requires five inner steps and classifier_lr=0.01")
    if config["learned_encoder_lr"] != dict(minimum=.001, maximum=.05, initial=.01):
        raise ValueError("Requires fixed learned LR bounds and initialization")
    method = config["method_config"]
    if config["method"] != "fo-proto-ema-learnedlr-lrsgmaml" or method["first_order"] is not True:
        raise ValueError("This baseline requires fo-proto-ema-learnedlr-lrsgmaml and first_order=true")
    for key in ("reset_classifier",):
        if method[key] is not False:
            raise ValueError(f"FO-Proto-EMA-LearnedLR-LRSGMAML requires {key}=false")
...
class MyMetaLearner(MetaLearner):
    def __init__(self, train_classes, total_classes, logger):
        ...
        self.inner_lrs = LearnedInnerLR(self.meta_learner, self.params["inner_steps"])
        # EMA buffers are never optimized; LR logits share the existing Adam.
        self.meta_parameters = self.weights + list(self.transport.parameters()) + list(self.inner_lrs.parameters())
        self.logger = logger
    ...
    def meta_fit(self, meta_train_generator, meta_valid_generator):
        ...
        for i, task in enumerate(meta_train_generator(exp["train_iterations"])):
            ...
            adapted = adapt(self.meta_learner, self.weights, support.to(self.dev),
                            ...
                            inner_lrs=self.inner_lrs, return_task_embedding=use_history)
            ...
            if (i + 1) % exp["validate_every"] == 0:
                log_metrics(self.logger, self.transport, i + 1, self.inner_lrs)
                self.meta_valid(meta_valid_generator)
        if self.transport._count:
            log_metrics(self.logger, self.transport, exp["train_iterations"], self.inner_lrs)
    ...
    def meta_valid(self, generator):
        ...
        if score > self.best_score:
            ...
            self.best_state = snapshot(self.meta_learner, self.transport, self.inner_lrs)
...
class MyLearner(Learner):
    ...
    def _initialize(self):
        ...
        self.inner_lrs = LearnedInnerLR(self.learner, self.config["method_config"]["inner_steps"])
        if self.inner_lrs.layout() != self.state["inner_lr_layout"]:
            raise ValueError("Checkpoint inner LR layout mismatch")
        self.inner_lrs.load_state_dict(self.state["inner_lrs"], strict=True)
        self.inner_lrs.eval()
        self.transport.eval()
        self.learner.eval()
    ...
    def save(self, path_to_save):
        ...
        torch.save(dict(format_version=1, method="fo-proto-ema-learnedlr-lrsgmaml",
    ...
    def load(self, path_to_load):
        ...
        if data["method"] != "fo-proto-ema-learnedlr-lrsgmaml" or data["format_version"] != 1:
            raise ValueError("Unsupported FO-Proto-EMA-LearnedLR-LRSGMAML checkpoint")
