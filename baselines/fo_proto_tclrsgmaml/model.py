...
def validate_config(config):
    ...
    for key in ("reset_classifier",):
        if method[key] is not False:
            raise ValueError(f"FO-Proto-TCLRSGMAML requires {key}=false")
...
class MyMetaLearner(MetaLearner):
    def __init__(self, train_classes, total_classes, logger):
        ...
        self.transport = TaskConditionedTransport(self.meta_learner, self.config)
...
class MyLearner(Learner):
    ...
    def _initialize(self):
        ...
        self.transport = TaskConditionedTransport(self.learner, self.config)
        if self.transport.architecture() != self.state["architecture"]:
            raise ValueError("Checkpoint TCLRSG architecture mismatch")
    ...
    def save(self, path_to_save):
        ...
        torch.save(dict(format_version=1, method="fo-proto-tclrsgmaml",
