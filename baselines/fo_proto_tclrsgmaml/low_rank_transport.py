...
class LowRankTransport(nn.Module):
    ...
    def transport_gradient(self, name, grad, conditioning=None):
        ...
        if not self.enabled:
            return grad
        key = self.indices[name]
        delta_a, delta_c = (0, {}) if conditioning is None else conditioning
        scalar_delta = delta_a if conditioning is None else delta_a[int(key)]
        transformed = (self.logits[key] + scalar_delta).sigmoid() * grad
        if key in self.u:
