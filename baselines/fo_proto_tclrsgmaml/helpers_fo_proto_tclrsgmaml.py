...
def adapt(model, weights, support, labels, config, num_classes=None, transport=None):
    ...
    # Reuse the prototype support forward; no conditioning path into encoder.
    conditioning = transport.condition(features.mean(dim=0).detach()) if transport is not None else None
