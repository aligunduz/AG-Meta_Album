"""Paired inner-loop ablation for one FO-Proto-TCSGMAML checkpoint.
...
the support loss after the longest adaptation. For this model it also records
the realized task gates (mean, min, max over the 60 encoder tensors), computed
from the same detached support-feature mean the model uses. One CSV row per task.

Example (repository root):
    python -u baselines/fo_proto_tcsgmaml/eval_inner_steps.py \
...
from helpers_fo_proto_tcsgmaml import prototype_head  # noqa: E402
...
def support_diagnostics(learner, support, labels, ways, adapted):
    ...
    with torch.no_grad():
        ...
        gates = learner.task_gates(features.mean(dim=0)).float()
    ...
    return (loss_start.item(), grad_norm, loss_after.item(),
            gates.mean().item(), gates.min().item(), gates.max().item())
...
def main():
    ...
    fields = (["task_id", "dataset", "num_ways", "num_shots"]
              ...
              + ["support_loss_start", "encoder_grad_norm_start",
                 f"support_loss_after{longest}", "gate_mean", "gate_min", "gate_max"])
    ...
    with open(args.out_csv, "w", newline="") as handle:
        ...
        for i, task in enumerate(loader.generator(args.test_tasks_per_dataset)):
            ...
            start, grad_norm, after, g_mean, g_min, g_max = support_diagnostics(
            ...
            row.update(gate_mean=g_mean, gate_min=g_min, gate_max=g_max)
            writer.writerow(row)
