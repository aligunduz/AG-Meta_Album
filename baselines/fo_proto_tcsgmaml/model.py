"""Shared-Embedding FO-Proto-TCSGMAML; controlled FO-Proto-MAML extension."""
import json
import random
...
from helpers_fo_proto_tcsgmaml import adapt, query_loss
from task_gates import TaskGates
...
def validate_config(config):
    ...
    if config["method"] != "fo-proto-tcsgmaml" or method["first_order"] is not True:
        raise ValueError("Requires fo-proto-tcsgmaml and first_order=true")
    if method["task_conditioned_gate"] is not True or "task_encoder" in config:
        raise ValueError("Requires shared-embedding task gates, without a task encoder")
    for key in ("gradient_transport", "low_rank_transport",
                "reset_classifier"):
        if method[key] is not False:
            raise ValueError(f"FO-Proto-TCSGMAML requires {key}=false")
    if config["test_checkpoint"] != "max-va.pth":
        raise ValueError("Test must use max-va.pth")
    if method["inner_steps"] < 1 or method["meta_batch_size"] < 1:
        raise ValueError("Positive inner_steps and meta_batch_size required")
    # The reference protocol uses complete summed meta-batches. Reject an
    # incomplete configuration rather than silently discarding its gradients.
    if config["experiment_config"]["train_iterations"] % method["meta_batch_size"]:
        raise ValueError("train_iterations must be divisible by meta_batch_size")
...
def snapshot(encoder, task_gates):
    return dict(encoder=cpu_state(encoder), task_gates=cpu_state(task_gates),
                gate_architecture=task_gates.architecture())
...
class MyMetaLearner(MetaLearner):
    def __init__(self, train_classes, total_classes, logger):
        ...
        self.task_gates = TaskGates(self.meta_learner, self.config)
        self.meta_parameters = (self.weights + list(self.task_gates.shared_logits)
                                + list(self.task_gates.gate_net.parameters()))
    ...
    def meta_fit(self, meta_train_generator, meta_valid_generator):
        ...
        for i, task in enumerate(meta_train_generator(exp["train_iterations"])):
            ...
            fast = adapt(self.meta_learner, self.weights, support.to(self.dev),
                         labels.to(self.dev), self.params, self.task_gates, task.num_ways)
            ...
            for j, w in enumerate(self.meta_parameters):
                grad = w.grad
                if grad is None:
                    raise RuntimeError("A persistent parameter is disconnected from query loss")
    ...
    def meta_valid(self, generator):
        ...
        if score > self.best_score:
            ...
            self.best_state = snapshot(self.meta_learner, self.task_gates)
...
class MyLearner(Learner):
    ...
    def _initialize(self):
        ...
        self.task_gates = TaskGates(self.learner, self.config)
        if self.state["gate_architecture"] != self.task_gates.architecture():
            raise ValueError("Checkpoint gate architecture/encoder ordering mismatch")
        self.task_gates.load_state_dict(self.state["task_gates"], strict=True)
        self.task_gates.eval()
        self.learner.eval()
    ...
    def fit(self, support_set):
        ...
        fast = adapt(self.learner, list(self.learner.parameters()),
                     ...
                     self.config["method_config"], self.task_gates, ways)
    ...
    def save(self, path_to_save):
        ...
        torch.save(dict(format_version=1, method="fo-proto-tcsgmaml",
    ...
    def load(self, path_to_load):
        ...
        if data["method"] != "fo-proto-tcsgmaml" or data["format_version"] != 1:
            raise ValueError("Unsupported FO-Proto-TCSGMAML checkpoint")
