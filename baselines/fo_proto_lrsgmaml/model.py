"""Independent FO-Proto-LRSGMAML submission using the repository FOMAML protocol."""
import json
import random
...
from helpers_fo_proto_lrsgmaml import adapt, query_loss
...
def validate_config(config):
    ...
    if config["method"] != "fo-proto-lrsgmaml" or method["first_order"] is not True:
        raise ValueError("This baseline requires fo-proto-lrsgmaml and first_order=true")
    for key in ("task_conditioned_gate", "reset_classifier"):
        if method[key] is not False:
            raise ValueError(f"FO-Proto-LRSGMAML requires {key}=false")
...
class MyMetaLearner(MetaLearner):
    def __init__(self, train_classes, total_classes, logger):
        ...
        self.transport = LowRankTransport(self.meta_learner, self.config["lrsg"])
...
class MyLearner(Learner):
    ...
    def _initialize(self):
        ...
        self.transport = LowRankTransport(self.learner, self.config["lrsg"])
        if self.transport.architecture() != self.state["architecture"]:
            raise ValueError("Checkpoint LRSG architecture mismatch")
    ...
    def save(self, path_to_save):
        ...
        torch.save(dict(format_version=1, method="fo-proto-lrsgmaml",
    ...
    def load(self, path_to_load):
        ...
        if data["method"] != "fo-proto-lrsgmaml" or data["format_version"] != 1:
            raise ValueError("Unsupported FO-Proto-LRSGMAML checkpoint")
